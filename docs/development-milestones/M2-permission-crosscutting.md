...
- [x] 创建 `service/attachment/ClubAttachmentService.java`。
- [x] 创建 `service/attachment/ClubAttachmentServiceImpl.java`。
- [x] 复用 infra 文件能力。
- [x] 写附件绑定记录。
- [x] 支持业务对象类型和 ID。
- [x] 支持附件锁定。
...
- RED：新增 `ClubAttachmentServiceImplTest` 后运行 `mvn -pl yudao-module-clubpoints -am -Dtest=ClubAttachmentServiceImplTest "-Dsurefire.failIfNoSpecifiedTests=false" "-Dflatten.skip=true" test`，失败于 `ClubAttachmentService`、`ClubAttachmentServiceImpl`、`ClubAttachmentRefDO`、`ClubAttachmentRefMapper`、`ClubAttachmentBindReqBO`、`ClubAttachmentConstants`、`CLUB_ATTACHMENT_LOCKED` 不存在；测试有效。
- GREEN：新增 `ClubAttachmentConstants`、`ClubAttachmentRefDO`、`ClubAttachmentRefMapper`、`ClubAttachmentBindReqBO`、`ClubAttachmentService`、`ClubAttachmentServiceImpl`，补 `CLUB_ATTACHMENT_INVALID`、`CLUB_ATTACHMENT_NOT_EXISTS`、`CLUB_ATTACHMENT_LOCKED` 错误码。
- 绑定实现：文件附件调用 `FileService.getFile(fileId)` 校验 infra 文件存在，只写 `club_points_attachment_ref`，不调用 `createFile` 复制文件；外部链接附件不调用 `FileService`。
- 锁定实现：`lockBizAttachments(bizType,bizId)` 只锁定 `status = 1` 的有效附件，写入 `locked = true` 和 `lock_time`，用于审核通过后冻结材料附件证据。
- 删除实现：`validateCanDelete` 对已锁定附件抛 `CLUB_ATTACHMENT_LOCKED`；普通删除只把绑定记录状态置为 `3`，不提供物理删除材料能力。物理删除材料仍必须由后续材料业务服务调用 M2.4 `ClubAuditService` 写强审计，M2.5 不新增绕过强审计的物理删除入口。
- 测试适配：`BaseMapperX` 不提供 `insertAndGetId`，测试改为 `insert` 后读取 DO 回填 ID，未为测试污染生产 Mapper。
- 单测验证：`mvn -pl yudao-module-clubpoints -am -Dtest=ClubAttachmentServiceImplTest "-Dsurefire.failIfNoSpecifiedTests=false" "-Dflatten.skip=true" test` 返回 `BUILD SUCCESS`；`ClubAttachmentServiceImplTest` 运行 `5` 个测试，失败 `0`，错误 `0`。
- 组合验证：`mvn -pl yudao-module-clubpoints -am "-Dtest=DictTypeConstantsTest,ClubScopeServiceImplTest,ClubAuditServiceImplTest,ClubAttachmentServiceImplTest" "-Dsurefire.failIfNoSpecifiedTests=false" "-Dflatten.skip=true" test` 返回 `BUILD SUCCESS`；合计 `22` 个测试，失败 `0`，错误 `0`。
- 质量验证：`git diff --check` 无空白错误；`rg -n "tenant_id|TenantBaseDO" yudao-module-clubpoints` 无命中；`rg -n "Co-authored|co-author|AI-generated|Generated by AI|ChatGPT" yudao-module-clubpoints` 无命中；`rg -n "createFile\(" yudao-module-clubpoints/src/main/java/cn/iocoder/yudao/module/clubpoints/service/attachment` 无命中。

## 任务 M2.6 ClubNotifyService

- [x] 创建 `service/notify/ClubNotifyService.java`。
- [x] 创建 `service/notify/ClubNotifyServiceImpl.java`。
- [x] 复用 system 站内信能力。
- [x] 封装活动审核结果通知。
- [x] 封装积分到账通知。
- [x] 封装兑换审核通知。
...
- RED：新增 `ClubNotifyServiceImplTest` 和 `ClubNotifyTemplateConstantsTest` 后运行 `mvn -pl yudao-module-clubpoints -am "-Dtest=ClubNotifyServiceImplTest,ClubNotifyTemplateConstantsTest" "-Dsurefire.failIfNoSpecifiedTests=false" "-Dflatten.skip=true" test`，失败于 `ClubNotifyService`、`ClubNotifyServiceImpl`、`ClubNotifyTemplateConstants` 不存在；测试有效。
- GREEN：新增 `ClubNotifyTemplateConstants`、`ClubNotifyService`、`ClubNotifyServiceImpl`，封装活动审核结果、积分变动、兑换审核结果、异议回复四类通知；统一调用 `NotifySendService.sendSingleNotifyToAdmin`。
- Seed 修正：`club-points-seed.sql` 补 `club_points_activity_reviewed` 活动审核结果通知模板；模板常量测试保证 Java 常量和 seed 中 `club_points_*` 通知模板 code 一致。
- 非阻断验证：`ClubNotifyServiceImpl` 捕获底层站内信异常并记录日志；`TransactionalProbeService.writeBusinessThenNotify()` 先写业务数据再触发通知异常，方法返回后业务插入仍存在，证明通知失败不回滚主业务。
- 单测验证：`mvn -pl yudao-module-clubpoints -am "-Dtest=ClubNotifyServiceImplTest,ClubNotifyTemplateConstantsTest" "-Dsurefire.failIfNoSpecifiedTests=false" "-Dflatten.skip=true" test` 返回 `BUILD SUCCESS`；合计 `7` 个测试，失败 `0`，错误 `0`。
...
- MySQL 验证：临时库 `club_points_m2_notify_green` 依次导入 `ruoyi-vue-pro.sql`、`club-points-schema.sql`、`club-points-seed.sql` 成功；重复导入 seed 成功；`system_notify_template` 中 `club_points_%` 通知模板为 `5` 个，包含 `club_points_activity_reviewed`、`club_points_activity_settled`、`club_points_dispute_replied`、`club_points_redemption_reviewed`、`club_points_transaction_changed`。
- 质量验证：`git diff --check` 无空白错误；`rg -n "tenant_id|TenantBaseDO" yudao-module-clubpoints` 无命中；`rg -n "Co-authored|co-author|AI-generated|Generated by AI|ChatGPT" yudao-module-clubpoints` 无命中。
