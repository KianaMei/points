...
如果基于 `C:\jobs\ruoyi` 落地，架构基线映射为：

- 一个应用后端：`yudao-server`。
- 一个新增业务模块：`yudao-module-clubpoints`。
- 组织、用户、角色、菜单和站内信：复用 `yudao-module-system`。
- 文件、定时任务、Excel 导出和代码生成：复用 `yudao-module-infra`。
- 安全、MyBatis、Redis、Lock4j、Quartz：复用 `yudao-framework`。
- 俱乐部范围权限、积分账本、兑换库存锁、业务强审计和业务任务运行记录：在 `yudao-module-clubpoints` 自定义。
...
flowchart TD
    accTitle: 积分系统总体架构
    accDescr: 展示模块化单体架构中的三类用户入口、应用服务、业务能力、横切能力、存储和调度器。
...
flowchart TB
    accTitle: 应用分层模型
    accDescr: 展示模块化单体内部如何保持控制器轻量，并把事务、领域规则和基础设施适配分层处理。
...
sequenceDiagram
    accTitle: 写用例调用序列
    accDescr: 展示关键写操作的标准路径，包括权限检查、事务边界、业务校验、持久化、审计和通知调度。
...
| 活动 | 创建草稿、提交审核、管理员审核、发布后修改、取消活动、配置签到签退窗口 | 关键修改和取消必须写审计；物理删除活动不进入当前 MVP |
| 报名签到 | 报名活动、取消报名、自助签到、自助签退、补录修正、标记特殊缺席 | 补录修正必须从报名记录发起，并校验权限范围 |
| 活动结算 | 结算基础参与积分、结算全程额外积分、结算缺席扣分、检查月度累计缺席 | 必须按活动和员工维度幂等 |
| 非签到积分 | 提交材料、撤回材料、重提材料、审核材料、管理员代录 | 审核通过要同时锁定材料并生成流水；物理删除材料不进入当前 MVP |
...
| 报表导出 | 生成积分明细、兑换记录、总台账、俱乐部排名、预算统计 | 只读业务数据，必须记录导出审计 |
...
| `ClockPort` | 提供当前时间和业务时区 | 业务时间和年度边界统一使用北京时间（Asia/Shanghai） |
...
需求和架构都不按编号模块拆。这里按业务域和支撑能力描述边界，用来指导事务、事实源和依赖方向。

### 业务域边界

| 业务域 | 主要职责 | 不负责 |
| --- | --- | --- |
| 俱乐部与成员 | 俱乐部创建、停用、删除、成员加入退出、负责人维护、成员范围 | 活动签到、兑换、积分余额计算 |
| 活动参与与结算 | 活动草稿、审核、发布、取消、报名、签到签退、修正、活动结算、缺席扣分 | 保存积分余额、直接改写账本流水 |
| 积分账本与个人查账 | 流水、冻结、撤销、调整、余额推导、来源统计、俱乐部发放统计口径 | 活动报名事实、兑换申请状态 |
| 非签到积分 | 材料提交、撤回、重提、审核、代录、材料快照、生成积分流水 | 基础参与积分、员工直接提交材料 |
| 兑换与发放 | 批次、礼品、资格快照、申请、库存锁定、积分冻结、审核、直接发放 | 领取、签收、质量问题 |
| 异议与调整 | 员工异议、管理员处理、回复、必要时触发账本调整或撤销 | 直接改写历史流水 |
| 年度清零、排名与激励 | 年度清零、跨年冻结口径、俱乐部排名、激励建议 | 预算审批和经费审批流 |

### 支撑能力边界

| 支撑能力 | 用途 | 边界 |
| --- | --- | --- |
| 组织、身份与权限 | 员工、部门、角色、菜单、数据范围、负责人范围 | 不计算积分余额 |
| 规则版本 | 发布、停用、公示、历史规则引用 | 不自动重算历史流水 |
| 附件与链接 | 文件上传、外部链接、附件锁定、业务绑定 | 不做业务审核决策 |
| 通知与待办 | 系统内通知、已读未读、负责人待办、管理员待办 | 不承载业务主状态 |
| 审计与后台任务 | 强审计、任务运行记录、失败重试、人工处理 | 不替代业务流水或状态机 |
| 报表与导出 | 明细、兑换、总台账、排名、预算统计、导出审计 | 只读，不反写业务状态 |
| 预算经费记录 | 预算分类、预算金额、实际支出、附件和备注 | 第一版不做审批流和自动预算压缩 |

### 能力依赖图

下图是压缩依赖视图。活动、非签到积分、兑换、异议和年度任务都围绕账本协作；附件、通知、审计是横切支撑。

```mermaid
flowchart LR
    accTitle: 能力依赖规则
    accDescr: 展示业务域如何调用账本、规则、附件、审计和通知能力，以及允许的依赖方向。
...
- 积分账本能力是积分事实源，其他能力只能通过账本用例生成流水、撤销流水、调整流水和冻结记录，不能直接改余额。
- 活动参与与结算能力可以请求账本生成基础参与积分、全程参与积分和缺席扣分，但活动侧不保存余额。
- 非签到积分能力可以在审核通过或管理员代录时请求账本生成流水，但材料本身不是流水。
- 兑换能力可以请求账本冻结积分、释放冻结、生成兑换扣减流水，但兑换申请状态不放进账本。
- 年度任务能力可以请求账本生成年度清零流水，但年度清零任务本身需要单独记录执行批次和幂等标识。
- 报表导出能力只读，不拥有业务状态。导出留痕写强审计，不新增导出主表，不反向改变业务结果。
- 附件、通知、审计是横切能力。业务能力可以引用它们，但附件、通知、审计不能承载业务主状态。

## 权限和数据范围

完整的角色权限矩阵、功能清单、数据范围细则和强确认/强审计清单见 `club-points-functions-and-permissions.md`（功能层单一来源）。架构层只保留一条实现约束：

- 权限判断必须两层——先判角色能否做该动作，再判该动作是否落在允许的数据范围内。
- 两层都在应用用例层（后端）完成，不能只依赖前端隐藏按钮。
...
- 通知失败：业务结果不回滚；通知只是告知，如何重试属于实施细节，不在功能范围内。
- 附件上传失败：依赖附件的业务提交失败；非必填附件失败不影响业务草稿保存。
- 导出失败：不改变业务数据，只记录失败导出审计或错误原因。
...
| 年度清零 | 每年 1 月 1 日 00:00:00 北京时间（Asia/Shanghai） | 只清未冻结可用积分，冻结兑换不参与 |
...
stateDiagram-v2
    accTitle: 后台任务状态机
    accDescr: 展示定时任务从创建、运行、重试、成功、失败到人工处理的生命周期。
...
erDiagram
    accTitle: 概念数据边界
    accDescr: 展示进入物理数据库设计前必须保持分离的主要业务事实和概念关系。
...
快照不是为了替代主表，而是为了允许物理删除后历史仍然可读。只要某条流水、报表、审计或历史页面需要展示某个名称、时间、归属或材料摘要，就不能只依赖主表关联查询。
...
| 审计日志 | 主对象删除或修改后无法判断操作内容 | 操作前后快照、操作者、目标对象快照 |
| 导出审计 | 后续筛选条件或数据变化后无法复核导出 | 导出类型、筛选条件、导出人、导出时间 |
