...
这个系统不能按页面一页页堆。正确顺序是：**源码地基 -> 数据库设计 -> 规则配置 -> 权限范围 -> 账本 -> 俱乐部成员 -> 活动签到 -> 自动结算 -> 非签到积分和扣分 -> 兑换 -> 年度运营 -> 报表审计前端收口 -> 验收硬化 -> 可用性契约硬化**。
...
PUBLISHED -> CANCELLED
PUBLISHED -> FINISHED
DELETED                  (当前 MVP 不开放，后续恢复需补服务层、接口、快照和审计)
```
...
1. 负责人建草稿。
2. 负责人提交审核。
3. 负责人撤回待审活动不进入当前 MVP；驳回后可修改并重新提交。
4. 管理员审核通过发布。
5. 管理员驳回，负责人可修改重提。
6. 管理员可直接创建并发布。
7. 活动普通信息发布后可直接改。
...
M11.10 已落地管理员活动和材料页面：`/clubpoints/admin/activity`、`/clubpoints/admin/contribution-review`、`/clubpoints/admin/contribution-direct`。活动页支持管理员直接发布和审核负责人活动；材料审核页只读材料内容并提交审核结果；管理员代录页提交 `requestNo`、员工、积分、规则版本、原因和附件，失败重试保留同号。
...
M12.7 放行演示必须按这个顺序，可通过 `ClubPointMvpDemoHardeningTest` 重复准备和验证，不允许手工改库：

1. 管理员发布规则版本，调整一个区间规则。
2. 管理员创建俱乐部，添加员工成员，设置负责人。
3. 负责人创建活动并提交审核。
4. 管理员审核活动通过。
5. 员工报名活动。
6. 员工签到、签退。
7. 管理员以 `force=true` 手动触发活动结算，已发布活动先收口到已结束，再走原 Job 和结算链路。
8. 员工查看积分到账和活动结算流水。
9. 管理员创建兑换批次和礼品，开启批次并上架礼品。
10. 员工提交兑换申请，冻结积分和锁库存。
11. 管理员审核兑换通过，生成兑换扣减流水并直接发放。
12. 员工查看兑换扣分后的可用积分。
13. 管理员生成俱乐部年度排名。
14. 管理员执行年度清零。
15. 管理员查看审计日志和积分明细报表。

扩展流程不作为 M12.7 放行演示的强制步骤，但已经由前置里程碑测试和 M12 回归覆盖：

1. 非签到材料提交和审核由 M8 / M11 / M12.6 覆盖。
2. 违规扣分、弄虚作假处理和管理员代录由 M8 覆盖。
3. 员工异议、管理员处理、激励和预算由 M10 / M11 覆盖。
4. 报表导出和任务运行处理由 M11.4、M11.5、M11.13、M12.6 覆盖。
5. 活动物理删除、材料物理删除和负责人活动撤回不进入当前 MVP；后续恢复时必须重新补服务层、接口、前端入口、强审计和回归测试。

### 17.4 完成标准

- [x] 演示脚本全通过。
- [x] 必跑测试矩阵全部通过。
- [x] 没有硬编码制度分值；业务分值通过规则版本、规则项、活动配置或请求校验进入账本。
- [x] 所有积分变化都能从流水追溯。
- [x] 所有强审计动作都能查到审计。
- [x] 所有报表字段都能从流水、快照或业务记录生成。

### 17.5 未进入 MVP 和后续阶段

当前 MVP 不进入范围：

1. 活动物理删除、非签到材料物理删除、负责人活动撤回。
2. 独立审批流、预算审批流、复杂绩效系统联动。
3. 异步大数据量报表导出任务化；当前导出保留筛选和强审计，后续按数据量扩展。
4. 外部组织系统深度同步；当前员工、部门、手机号来源复用 `AdminUserApi` 和 `DeptApi`，成员关系保存必要快照。

后续阶段如恢复活动或材料物理删除，必须补齐 Service、Controller、前端入口、权限、强审计、历史快照、报表历史可读和回归测试，不能只加按钮或 API wrapper。

当前已知风险：

1. 前端工程仍有非 clubpoints 存量 TypeScript 债；M12 仅证明 `src/api/clubpoints` 和 `src/views/clubpoints` 路径过滤无新增类型错误。
2. 当前报表导出适合 MVP 数据量；如果达到 50 万流水量级，需要进入异步导出或强筛选窗口。
3. M12.7 API 演示覆盖 MVP 主闭环；非签到材料、违规扣分、弄虚作假、异议、预算、任务处理由对应里程碑和 M12 回归测试覆盖，不作为演示脚本强制步骤。

---

## 18. M13 可用性与前后端契约硬化

M13 是 M12 之后的真实可用性收口，不改变 M0-M12 的业务架构，不把已完成的主闭环推倒重做。它只处理 M11/M12 暴露出的工程验收缺口：

1. 前端 API wrapper 存在但后端 mapping 缺失。
2. 页面按钮看起来可点，但没有真实后端业务或缺少完整反馈。
3. 三类角色菜单命名不够清楚。
4. 页面把 `ID`、`runKey`、`requestNo`、`resultJson` 等数据库或研发语言暴露给业务用户。
5. M12 回归只覆盖主路径和已暴露死入口，没有把 API wrapper、页面按钮、权限 seed、Controller mapping 做成全量机器门禁。

M13 的执行入口是 `docs/development-milestones/M13-usability-contract-hardening.md`。放行时必须同时满足：

- `src/api/clubpoints/**/*.ts` 每个 `request.*` 都有后端 Controller mapping。
- `src/views/clubpoints/**/*.vue` 直接 `request.*` 和业务 URL 散写命中 0。
