...
| 详情路径 | `/get` |
| 分页路径 | `/page` |
| 删除路径 | 当前 MVP 仅实现俱乐部物理删除 `/delete`；活动和材料物理删除不开放 |
| 导出路径 | `/export-excel` |
...
| 场景 | 幂等键建议 | 要求 |
| --- | --- | --- |
| 活动积分发放 | `ACTIVITY_SETTLEMENT:{activityId}:{userId}:{itemType}` | 重跑不能重复发分 |
...
| 活动状态 | `1` 草稿，`2` 待审核，`3` 已驳回，`4` 已发布，`5` 已取消，`6` 已结束，`7` 已结算，`8` 已删除快照 | 员工只看已发布活动 |
...
| 活动积分发放状态 | `1` 待发放，`2` 发放中，`3` 已发放，`4` 发放失败，`5` 人工处理 | 后台任务可重试 |
...
| `CLUB_ACTIVITY_NOT_FOUND` | 活动不存在或已删除 | 提示并刷新列表 |
| `CLUB_ACTIVITY_TIME_INVALID` | 活动开始结束、报名截止、签到或签退窗口配置无效 | 提示修正活动时间 |
| `CLUB_ACTIVITY_LEVEL_INVALID` | 活动等级不是小型、中型或大型 | 提示选择有效活动等级 |
| `CLUB_ACTIVITY_STATUS_INVALID` | 活动状态不允许当前动作，包含未发布访问、报名、审核状态不匹配、已取消签到或已完成积分发放后关键字段修改 | 刷新详情或提示不可操作 |
| `CLUB_ACTIVITY_REGISTRATION_CLOSED` | 当前时间超过报名截止 | 禁止报名 |
| `CLUB_ACTIVITY_CANCEL_WINDOW_CLOSED` | 当前时间超过取消窗口 | 禁止取消报名 |
| `CLUB_ACTIVITY_REGISTRATION_DUPLICATED` | 员工重复报名同一活动 | 提示已报名并刷新报名状态 |
| `CLUB_ACTIVITY_REGISTRATION_NOT_FOUND` | 报名记录不存在、已取消或不属于本人 | 提示刷新报名记录 |
| `CLUB_ATTENDANCE_WINDOW_CLOSED` | 不在签到或签退窗口 | 提示窗口时间 |
| `CLUB_ATTENDANCE_ALREADY_EXISTS` | 重复签到或签退 | 显示已有记录 |
| `CLUB_ATTENDANCE_CHECKIN_REQUIRED` | 员工签退前没有有效签到事实 | 提示先签到 |
| `CLUB_ATTENDANCE_NOT_FOUND` | 管理员修正的签到或签退记录不存在 | 提示刷新签到记录 |
| `CLUB_ATTENDANCE_TARGET_INVALID` | 签到签退目标类型不是签到或签退 | 提示重新选择类型 |
| `SETTLEMENT_ALREADY_DONE` | 活动已完成积分发放，不能改原流水 | 引导走调整或撤销 |
...
| `todoItems` | List<DashboardTodoItemRespVO> | 待办项，点击按 `path` 和 `queryJson` 跳转 |

`AppNotifyMessageRespVO`：

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `id` | Long | 通知 ID |
| `templateCode` | String | 通知模板编码 |
| `templateNickname` | String | 通知发送昵称 |
| `templateContent` | String | 通知内容 |
| `templateType` | Integer | 通知类型 |
...
| `settlementStatus` | Integer | 积分发放状态 |
...
签到签退从本人报名记录进入。后端按 `registrationId` 加载本人报名记录和对应活动窗口，并按服务端北京时间（Asia/Shanghai）判断窗口，不能信任 `clientTime`。签退必须已有有效签到事实，避免 M7 结算读取到“无签到但有签退”的脏数据。
...
礼品分页只用于展示当前批次可见礼品；员工没有资格快照或不具备资格时返回空分页，不抛“资格快照不存在”这类后台处理错误。提交兑换申请时，后端必须同事务完成资格校验、可用积分校验、冻结积分、锁定库存、创建申请。库存不足时，前端显示“已兑完/库存不足”，不能继续提交。
...
负责人账本范围：负责人只能查询自己负责俱乐部作为 `issuingClubId` 发放来源产生的积分流水，以及基于该来源汇总出的成员积分摘要；成员积分摘要不是员工全量积分账户汇总。这里“他人/其他”不是指本俱乐部分发给成员的积分，而是指其他俱乐部、无俱乐部来源或全局调整来源；即使某员工是该俱乐部成员，负责人也不能通过本接口查看该员工这些来源下的流水原因、材料和来源快照。
...
| `count` | Integer | 待办数量 |
| `path` | String | 前端跳转路径 |
| `queryJson` | String | JSON 字符串形式的跳转筛选条件 |
...
提交发布审核接口当前使用 query 参数 `id`，不接收请求体；提交原因由后端写入默认操作原因。

`ActivitySubmitReqVO` 是历史设计名，当前 MVP 不暴露为请求体：
...
`ActivityCancelReqVO` 当前使用 query 参数：
...
当前 MVP 不开放负责人撤回待审活动和物理删除活动接口。后续阶段如恢复活动物理删除，必须回到 Service 层补快照、强审计、报表历史可读和接口测试。
...
目前所有非签到类积分材料都由俱乐部负责人提交，管理员审核。员工不提交这类材料。当前 MVP 不开放负责人或管理员物理删除材料接口；审核前材料可以撤回或修改，审核通过后只能通过管理员撤销流水或调整流水修正积分。
...
| 功能 | 方法 | 路径 | 权限 | 范围校验 |
| --- | --- | --- | --- | --- |
| 管理员工作台汇总 | `GET` | `/clubpoints/admin/dashboard/summary` | `clubpoints:dashboard:query` | 管理员 |

`AdminDashboardSummaryRespVO`：

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `pendingActivityReviewCount` | Integer | 待审核活动数量 |
| `pendingContributionReviewCount` | Integer | 待审核材料数量 |
| `pendingRedemptionReviewCount` | Integer | 待审核兑换数量 |
| `pendingDisputeCount` | Integer | 待处理异议数量 |
| `todoCount` | Integer | 待办数量 |
| `todoItems` | List<DashboardTodoItemRespVO> | 待办项，点击按 `path` 和 `queryJson` 跳转 |
...
| 设置负责人 | `POST` | `/clubpoints/club-leader/assign` | `clubpoints:club-leader:update` | 管理员 |
| 移除负责人 | `POST` | `/clubpoints/club-leader/remove` | `clubpoints:club-leader:update` | 管理员 |
| 成员分页 | `GET` | `/clubpoints/club-member/page` | `clubpoints:club-member:query` | 管理员 |
| 负责人分页 | `GET` | `/clubpoints/club-leader/page` | `clubpoints:club-leader:update` | 管理员 |
...
| `sort` | Integer | 否 | 排序 |
| `remark` | String | 否 | 备注 |
| `reason` | String | 否 | 创建或修改原因，后端写入审计上下文 |

`AdminClubOperationReqVO`：

| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `id` | Long | 是 | 俱乐部 ID |
| `reason` | String | 否 | 停用原因 |

`AdminClubDeleteReqVO`：
...
| `pageNo` | Integer | 是 | 页码 |
| `pageSize` | Integer | 是 | 每页条数 |
| `keyword` | String | 否 | 俱乐部名称关键词 |
| `status` | Integer | 否 | 俱乐部状态 |

`AdminClubMemberSaveReqVO` 用于设置 / 移除负责人、添加 / 移除成员：

| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `clubId` | Long | 是 | 俱乐部 ID |
| `userId` | Long | 是 | 负责人或成员用户 ID |
| `reason` | String | 否 | 任免、添加或移除原因 |

允许多个负责人，由系统管理员维护。设置负责人时用户必须是有效系统用户，但不要求先成为俱乐部普通成员。负责人不能自行新增、移除或转授权其他负责人；设置和移除负责人都必须写审计，移除后负责人数据范围立即收缩。

`ClubMemberPageReqVO` / `ClubLeaderPageReqVO`：

| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `pageNo` | Integer | 是 | 页码 |
| `pageSize` | Integer | 是 | 每页条数 |
| `clubId` | Long | 否 | 俱乐部 ID |
| `userId` | Long | 否 | 用户 ID |
| `status` | Integer | 否 | 成员或负责人状态 |
...
| 管理员补录签到签退 | `POST` | `/clubpoints/attendance/supplement` | `clubpoints:attendance:correct` | 管理员 |
| 管理员修正签到签退 | `POST` | `/clubpoints/attendance/correct` | `clubpoints:attendance:correct` | 管理员 |
| 管理员标记特殊缺席 | `POST` | `/clubpoints/registration/mark-special-absence` | `clubpoints:registration:special-absence` | 管理员 |

管理员创建活动可以直接发布，也可以保存草稿。

管理员补录和修正签到签退必须写强审计；补录写有效签到签退事实和修正历史，修正保留原有效记录 ID 并写 before/after 修正历史。特殊缺席不拆独立表，只更新活动报名记录上的特殊缺席、不扣缺席、原因、时间和操作人字段，供 M7 结算读取。M6 阶段这些动作不生成积分流水。
...
| 功能 | 方法 | 路径 | 权限 | 范围校验 |
| --- | --- | --- | --- | --- |
| 异常补发或重跑活动积分 | `POST` | `/clubpoints/settlement/run` | `clubpoints:settlement:run` | 管理员 |
| 发放记录分页 | `GET` | `/clubpoints/settlement/page` | `clubpoints:settlement:query` | 管理员 |
...
| --- | --- | --- | --- |
| `activityId` | Long | 是 | 活动 ID |
| `force` | Boolean | 否 | 是否强制触发，默认 `false`；`true` 仅允许管理员手动把已发布活动收口到已结束后生成积分 |
| `reason` | String | 是 | 触发原因 |

活动积分自动发放时间是签退窗口关闭后加缓冲时间。发放前，负责人和管理员可以修正签到签退记录。发放后，再修正签到签退只能通过补发、撤销或调整流水处理。

管理员异常补发或重跑活动积分时，`force=true` 不代表绕过积分发放链路或直接写流水。它只处理自动任务失败、签到 / 签退修正后补发、活动状态异常人工收口等场景：当活动仍为已发布状态时，后端先把活动状态推进为已结束，再进入原有 Job、积分计算、账本和幂等逻辑；`force=false` 时仍按原活动状态要求执行。
...
| 管理员违规扣分 | `POST` | `/clubpoints/contribution/violation-deduct` | `clubpoints:contribution:violation-deduct` | 管理员 |
| 管理员弄虚作假处理 | `POST` | `/clubpoints/contribution/fraud-handle` | `clubpoints:contribution:fraud-handle` | 管理员 |
...
审核前，负责人可以撤回或修改后重新提交。审核通过后不能修改，只能由管理员撤销或调整。材料物理删除不进入当前 MVP；管理员审核页只提供审核、代录、违规扣分和弄虚作假处理。
...
`ContributionViolationDeductReqVO`：

| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `requestNo` | String | 是 | 请求号 |
| `clubId` | Long | 是 | 扣分关联俱乐部 ID |
| `userId` | Long | 是 | 员工 ID |
| `points` | Integer | 是 | 扣分值 |
| `ruleVersionId` | Long | 是 | 规则版本 |
| `reason` | String | 是 | 违规原因 |
| `attachments` | List<AttachmentInputVO> | 是 | 材料 |

管理员违规扣分直接生效，必须走 `VIOLATION_DEDUCT_RANGE` 规则区间校验，通过 LedgerService 生成负向流水，不允许直接修改余额或原流水。

`ContributionFraudHandleReqVO`：

| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `requestNo` | String | 是 | 请求号 |
| `originalMaterialId` | Long | 是 | 原已审核通过材料 ID |
| `ruleVersionId` | Long | 是 | 规则版本 |
| `reason` | String | 是 | 弄虚作假处理原因 |
| `attachments` | List<AttachmentInputVO> | 是 | 处理材料 |

管理员弄虚作假处理直接生效，只能撤销原材料已发正向流水并按 `FRAUD_CLEAR_ALL` 扣除涉及员工当前全部可用积分；原流水不得删除或改写，同时取消涉及员工年度评优资格并通知员工。重复 `requestNo` 必须返回既有处理材料，不重复撤销或扣分。
...
| 资格快照分页 | `GET` | `/clubpoints/redemption-batch/eligibility-page` | `clubpoints:redemption-batch:manage` | 管理员 |
...
| 年度清零记录 | `GET` | `/clubpoints/annual/clearing-record-page` | `clubpoints:annual:query` | 管理员 |
| 生成年度排名 | `POST` | `/clubpoints/annual/ranking-generate` | `clubpoints:annual:manage` | 管理员 |
...
| 确认激励 | `POST` | `/clubpoints/annual/incentive-confirm` | `clubpoints:annual:manage` | 管理员 |
| 取消激励 | `POST` | `/clubpoints/annual/incentive-cancel` | `clubpoints:annual:manage` | 管理员 |
...
| 停用预算记录 | `POST` | `/clubpoints/budget/disable` | `clubpoints:budget:manage` | 管理员 |
| 积分明细报表分页 | `GET` | `/clubpoints/report/point-detail-page` | `clubpoints:report:query` | 管理员 |
| 积分总台账报表分页 | `GET` | `/clubpoints/report/ledger-summary-page` | `clubpoints:report:query` | 管理员 |
| 兑换记录报表分页 | `GET` | `/clubpoints/report/redemption-page` | `clubpoints:report:query` | 管理员 |
| 俱乐部排名报表分页 | `GET` | `/clubpoints/report/club-ranking-page` | `clubpoints:report:query` | 管理员 |
| 预算统计报表分页 | `GET` | `/clubpoints/report/budget-page` | `clubpoints:report:query` | 管理员 |
...
| 任务运行详情 | `GET` | `/clubpoints/job-run/detail` | `clubpoints:job:query` | 管理员 |
...
| `year` | Integer | 否 | 清零年度 |
| `userId` | Long | 否 | 员工 ID |
| `status` | Integer | 否 | 清零状态 |

`AnnualRankingGenerateReqVO`：

| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `year` | Integer | 是 | 排名年度 |

`AnnualRankingPageReqVO`：
...
`AnnualIncentiveSuggestReqVO`：

| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `year` | Integer | 是 | 激励建议来源年度 |

`AnnualIncentiveOperationReqVO`：

| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `id` | Long | 是 | 激励记录 ID |
| `reason` | String | 否 | 确认或取消说明 |
...
| `budgetAmountCent` | Long | 是 | 预算金额，单位分 |
| `actualAmountCent` | Long | 否 | 实际支出，单位分 |
| `occurDate` | LocalDate | 否 | 发生日期 |
| `handlerUserId` | Long | 否 | 经办人用户 ID |
| `sourceType` | Integer | 否 | 来源类型，默认手工 |
| `sourceId` | Long | 否 | 来源对象 ID |
| `description` | String | 否 | 预算说明 |
| `remark` | String | 否 | 备注 |
| `reason` | String | 否 | 创建或修改原因 |

`BudgetOperationReqVO`：
...
| `pageNo` | Integer | 是 | 页码 |
| `pageSize` | Integer | 是 | 每页条数 |
| `clubId` | Long | 否 | 发放俱乐部或排名俱乐部 |
| `userId` | Long | 否 | 员工 |
| `year` | Integer | 否 | 年度 |
| `status` | Integer | 否 | 兑换记录状态 |
| `category` | Integer | 否 | 预算分类 |
| `sourceType` | Integer | 否 | 预算来源或流水来源 |
| `startTime` | LocalDateTime | 否 | 开始时间，积分报表按 `occurred_at`，兑换报表按 `apply_time` |
| `endTime` | LocalDateTime | 否 | 结束时间，积分报表按 `occurred_at`，兑换报表按 `apply_time` |

报表查询只读事实源：积分明细读取 `club_points_transaction`；总台账读取 `club_points_transaction` 聚合并带出 `club_points_point_account` 当前缓存；兑换记录读取 `club_points_redemption_application` 的申请和礼品/批次快照，不用实时礼品可变字段覆盖历史；俱乐部排名读取 `club_points_annual_ranking_record`；预算统计读取 `club_points_budget_record`。员工和负责人没有报表导出权限。

`ReportExportReqVO`：

| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `reportType` | Integer | 是 | 1=积分明细、2=兑换记录、3=总台账、4=俱乐部排名、5=预算统计 |
| `clubId` | Long | 否 | 俱乐部 |
| `userId` | Long | 否 | 员工 |
| `year` | Integer | 否 | 年度 |
| `direction` | Integer | 否 | 流水方向，仅积分明细使用 |
| `pointCategory` | Integer | 否 | 积分分类，仅积分明细使用 |
| `sourceType` | Integer | 否 | 流水来源或预算来源 |
| `status` | Integer | 否 | 兑换状态，仅兑换记录使用 |
| `category` | Integer | 否 | 预算分类，仅预算统计使用 |
| `sourceId` | Long | 否 | 来源编号；兑换记录导出时表示批次编号 |
| `startTime` | LocalDateTime | 否 | 开始时间 |
| `endTime` | LocalDateTime | 否 | 结束时间 |
| `reason` | String | 否 | 导出原因，进入强审计 |

导出功能只有管理员能做，导出前由 `clubpoints:report:export` 校验权限，导出时必须写 `REPORT_EXPORT` 强审计，记录导出人、时间、类型、筛选条件和导出行数。强审计失败则导出失败。导出文件直接写 HTTP 响应，不新增导出业务主表。
...
2. 再做规则版本和账本最小接口，否则活动积分发放、非签到积分、兑换都没有事实源。
3. 再做俱乐部、成员、负责人接口。
