...
| `CLUB_ACTIVITY_STATUS_INVALID` | 活动状态不允许当前动作，包含未发布访问、报名、审核状态不匹配、已取消签到或已结算关键字段修改 | 刷新详情或提示不可操作 |
...
| --- | --- | --- | --- |
| `activityId` | Long | 是 | 活动 ID |
| `force` | Boolean | 否 | 是否强制触发，默认 `false`；`true` 仅允许管理员手动把已发布活动收口到已结束后触发结算 |
| `reason` | String | 是 | 触发原因 |

活动积分自动结算时间是签退窗口关闭后加缓冲时间。结算前，负责人和管理员可以修正签到签退记录。结算后，再修正签到签退只能通过补发、撤销或调整流水处理。

管理员手动触发结算时，`force=true` 不代表绕过结算链路或直接写流水。它只处理演示和人工收口场景：当活动仍为已发布状态时，后端先把活动状态推进为已结束，再进入原有 Job、结算 Service、账本和幂等逻辑；`force=false` 时仍按原结算状态要求执行。
